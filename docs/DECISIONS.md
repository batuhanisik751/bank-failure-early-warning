...
- 2026-09-26 — **Booster walk-forward regenerated at 8q on raw features.** Every
  `models/walkforward/<Y>/gbdt_8q/` artefact for 2008-2023 (the only 8q booster series;
  `gbdt_mono` has none and none was added) was refit with the bare `("model", estimator)`
  pipeline on raw registry features, the per-year tuning re-run on each year's own inner
  slice (`check_tuning_consistency`: zero violations, no iteration cap on any year; fits took
  about 4 s a year), and `walkforward_scores` rebuilt once (3,352,958 rows; 407,621 `gbdt` rows
  at 8q over 3,768 failures). Pooled 8q `gbdt`: PR-AUC 0.1059, recall@2% 0.2585 (winsorised:
  0.0887 / 0.2200). Per-year PR-AUC (winsorised in brackets): 2008 0.0326 (0.0326, still the
  degenerate `learning_rate 0.1` pick on an 11-positive validation slice, ROC-AUC 0.4999),
  2009 0.2389 (0.3836), 2010 0.3703 (0.2975), 2011 0.4486 (0.3618), 2012 0.4136 (0.4364),
  2013 0.3048 (0.2917), 2014 0.3521 (0.3634), 2015 0.3157 (0.3677), 2016 0.2936 (0.3605),
  2017 0.0305 (0.0419), 2018 0.2315 (0.2372), 2019 0.6082 (0.5549), 2020 0.6917 (0.6429),
  2021 0.0016 (0.0017), 2022 0.0028 (0.0049), 2023 0.0124 (0.0084). Recall@2% per year:
  0.0027, 0.2282, 0.3646, 0.7017, 0.8144, 0.7818, 0.9344, 0.8269, 0.7879, 0.5333, 0.7143,
  0.9545, 1.0, 0.0, 0.0370, 0.3158. The pooled gain comes from 2010-2011 and 2019-2020 and is
  offset by 2009 (a `learning_rate 0.1` pick that loses 0.14) and 2015-2016; six of sixteen
  years still choose `learning_rate 0.1`, and 2021-2023 remain near random at 8q for the
  unconstrained booster, so the 8q series stays the weakest ranking in the backtest (hazard
