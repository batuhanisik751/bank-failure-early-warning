...
def _intro_lines(settings: Settings) -> list[str]:
    ...
    return [
        ...
        "unconstrained gradient booster and `gbdt_mono` the same booster under the "
        "registry's monotone signs; `hazard` the one-quarter hazard converted with "
        "`1 - (1 - h)^H`. Pooled rows score every test year as one ranking. `brier raw` is "
        "the mean squared error of the model's own output (n/a for the Texas ranking); "
        "`brier calibrated` is that of the isotonic map fitted per year on the last complete "
        "label year inside the training window, scored by the year's own model for the "
        "logits and by an inner model trained on the years before the slice for the "
        "boosters, so no test outcome shapes the map. Generated by "
        "`bankcanary metrics-report`.",
        "",
    ]
...
def _calibration_lines(suites: dict[str, dict], horizon: int, figures: Path) -> list[str]:
    lines = [
        ...
        "training window (widened backwards while it holds fewer than five failures). The "
        "logit and the hazard score that slice with Y's own full-window model, so the map "
        "is learned on the score scale it is applied to (in-sample for those rows, which a "
        "regularised logit on hundreds of thousands of rows tolerates); the boosters "
        "separate their training rows perfectly and use an inner model fitted on the years "
        "before the slice instead, so their map is applied to a scale it never saw and can "
        "inherit the inner model's plateaus (`calibration.SLICE_SCORER_BY_MODEL`). Pooled "
        "deciles mix years, so a model whose raw scale drifts between years shows a "
        "non-monotone raw curve even when every single year ranks well.",
        "",
    ]
    for model, s in suites.items():
